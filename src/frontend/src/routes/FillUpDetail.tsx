...
function SyncBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    ...
    "not-configured": "bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400",
    skipped: "bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400",
  };
  return (
    <span
      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
        colors[status] ?? "bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400"
      }`}
    >
      {status}
    </span>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Fill-Up Details</h1>
      ...
      {/* Sync status */}
      <div className="rounded-xl bg-white dark:bg-gray-800 p-6 shadow-sm dark:shadow-gray-900/30">
        <h2 className="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">Sync Status</h2>
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-600 dark:text-gray-400">Paperless</span>
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-600 dark:text-gray-400">YNAB</span>
            <SyncBadge status={fillUp.ynabSyncStatus} />
          </div>
          {fillUp.ynabAccountName && (
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-600 dark:text-gray-400">YNAB Account</span>
              <span className="text-sm text-gray-900 dark:text-gray-100">{fillUp.ynabAccountName}</span>
            </div>
          )}
          {fillUp.ynabCategoryName && (
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-600 dark:text-gray-400">YNAB Category</span>
              <span className="text-sm text-gray-900 dark:text-gray-100">{fillUp.ynabCategoryName}</span>
            </div>
          )}
        </div>
      </div>
